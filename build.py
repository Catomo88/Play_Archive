...
@media (max-width: 768px) {
  ...
  .tab-content ul, .tab-content ol { margin: 0.6rem 0; padding-left: 1.2rem; }
  .tab-content li { margin: 0.55rem 0; line-height: 1.65; }
  /* 슬롯 칩(메인/서브/R/S/M)이 들어간 줄은 칩과 텍스트 사이 호흡 더 */
  .tab-content li:has(.slot-tag) { margin: 0.65rem 0; line-height: 1.7; }
  .tab-content table { font-size: 0.82rem; margin: 0.6rem 0; }
  .tab-content th, .tab-content td { padding: 0.35rem 0.5rem; }
  /* 표가 가로로 길어 화면을 넘으면 부모(info-card-body / .tab-content)가 가로 스크롤 */
  .info-card-body,
  .boss-card-body,
  .tab-content > .scrollable-table {
    overflow-x: auto;
    -webkit-overflow-scrolling: touch;
  }
  /* 표는 자기 본래 폭을 유지, 넘으면 부모가 스크롤 */
  .info-card-body table,
  .boss-card-body table,
  .tab-content > table {
...
.slot-tag.s-main { background: rgba(91, 158, 255, 0.18); color: #7cb3ff; border-left: 3px solid #5b9eff; }
.slot-tag.s-sub  { background: rgba(6, 214, 255, 0.15); color: #66e3ff; border-left: 3px solid #06d6ff; }
.slot-tag.s-r    { background: rgba(255, 77, 141, 0.16); color: #ff8aa9; border-left: 3px solid #ff4d8d; }
.slot-tag.s-s    { background: rgba(34, 197, 94, 0.16); color: #7cdda5; border-left: 3px solid #22c55e; }
.slot-tag.s-m    { background: rgba(255, 184, 0, 0.16); color: #ffd47a; border-left: 3px solid #ffb800; }

/* 모바일: 슬롯 칩 컴팩트 */
@media (max-width: 600px) {
  .slot-tag { font-size: 0.68rem; min-width: 2rem; padding: 0.03rem 0.35rem; }
}

/* ===== 캐릭터 서브탭 (캐릭터별 빌드 토글) ===== */
.char-builds { margin: 1rem 0; }
.char-subtabs {
...
.char-subtab:hover { color: var(--text-primary); border-color: var(--border-strong); }
.char-subtab.active {
  background: var(--accent-cyan); color: #001017;
  border-color: var(--accent-cyan);
  font-weight: 700;
}
.char-build-panel { margin-top: 1rem; }
.char-build-panel[hidden] { display: none; }

@media (max-width: 600px) {
  .char-subtabs { padding: 0.5rem; gap: 0.3rem; top: 56px; }
  .char-subtab { font-size: 0.78rem; padding: 0.3rem 0.65rem; }
}
